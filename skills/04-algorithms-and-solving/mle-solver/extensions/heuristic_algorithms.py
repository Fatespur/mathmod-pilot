...
def genetic_algorithm(
    ...
    for gen in range(n_generations):
        ...
        if verbose and gen % 20 == 0:
            print(f"GA Gen {gen:4d}: best={best_fitness:.6e}, avg={fitness.mean():.6e}")
        
        if stagnation_counter >= early_stop_generations:
            if verbose:
                print(f"GA 早停于第 {gen} 代，连续 {early_stop_generations} 代无改进")
            break
...
def particle_swarm_optimization(
    ...
    for it in range(n_iterations):
        ...
        if verbose and it % 20 == 0:
            print(f"PSO Iter {it:4d}: best={g_best_value:.6e}")
        
        if stagnation_counter >= early_stop_iterations:
            if verbose:
                print(f"PSO 早停于第 {it} 次迭代")
            break
...
def simulated_annealing(
    ...
    for restart in range(restart_count + 1):
        if restart > 0:
            ...
            if verbose:
                print(f"SA 重新加热 #{restart}，温度={temp:.1f}")
        ...
        if verbose:
            print(f"SA T={temp:.1f}: best={best_energy:.6e}, current={current_energy:.6e}")
...
def differential_evolution(
    ...
    for gen in range(n_generations):
        ...
        if verbose and gen % 20 == 0:
            print(f"DE Gen {gen:4d}: best={best_fitness:.6e}")
        
        if stagnation >= early_stop_generations:
            if verbose:
                print(f"DE 早停于第 {gen} 代")
            break
