...
For S3, `completed` means execution artifacts are complete and the release state is `WAITING_FOR_MODEL_VALIDATION`; it does not authorize S5–S7. S4 must bind its plan, evidence, failure envelope, and gate to the current model/data/selection hashes. `REVISE` and `FAIL` block presentation stages; `PASS_WITH_LIMITATIONS` permits them only with exact claim-restriction propagation.
...
| S1 | `problem-analyzer` | `problem_structure.json`、问题图、变量、约束、假设风险；禁止模型选择 |
| S2 | `data-processing` | 处理后数据、字段字典、质量报告 |
| Selection gate | `model-selection` | 候选组合、筛选矩阵、反模板门禁与选择结论 |
| S3 | `mle-solver` | 模型、代码、求解结果、日志、`validation_handoff.json`；状态为 `WAITING_FOR_MODEL_VALIDATION` |
| S4 | `model-validation` | 预注册计划、绑定证据、失效包络、四态独立门禁、修订令牌 |
