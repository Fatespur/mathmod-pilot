...
inputs: ["raw_or_simulated_data.csv", "chart_specification.json"]
outputs: ["publication_figure.png", "publication_figure.svg", "visual_qa_report.json"]
dependencies: ["scipilot-figure-cumcm"]
---

## Codex execution contract

- **Language:** Match the requested figure language. Default CUMCM labels to `zh-CN` and MCM/ICM labels to `en-US`; preserve symbols, units, column names, schema keys, paths, and code identifiers.
- **Inspect first:** Determine the claim, source data, column semantics, sample structure, target venue, final dimensions, and available runtime before choosing a chart.
- **Progressive disclosure:** Load `references/chart_selection.md` for selection, then only the recipe, journal, pitfall, or QA references needed by the active route.
- **Run, render, inspect:** Profile data, execute plotting code, render previews, run deterministic audits, visually inspect the image, revise, and export only after QA passes.
- **Evidence and privacy:** Trace each visual mark to a data snapshot; use local tooling unless external processing is explicitly authorized.
- **Handoff:** Use `pipeline_manifest.json`; on first pipeline use read [references/pipeline-contract.md](references/pipeline-contract.md) and [references/pipeline_manifest.schema.json](references/pipeline_manifest.schema.json). Record purpose, data, code, outputs, QA, warnings, and provenance.
- **Routing:** Send framework diagrams to `$cumcm-academic-flowchart`, MATLAB-native work to `$matlab-figure`, and journal assembly to `$nature-figure`.
- **Completion:** Mark final only after semantic, numerical, typography, crop, colorblind, and grayscale checks pass.
...
3. **视觉层（v2.1 新增）**：出图后**渲染 PNG → 程序自检 → AI 读图复核 → 回改**的闭环：
   ...
   - **使用当前可用的图像查看工具检查这张 PNG**，对照 `references/visual_review.md` 的 8 项清单核对
...
4. 若推荐图型会实质改变用户明确要求，再请求确认；否则直接进入第 3-7 步：查期刊规范、配环境、画、自检、导出
