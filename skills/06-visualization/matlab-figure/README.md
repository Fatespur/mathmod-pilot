# MATLAB Native 3D & Dynamic Visualization (`matlab-figure`)

> 所属分类：`06-visualization` | 竞赛流水线阶段：`S5A`

## 1. 技能概述 (Overview)

`matlab-figure` 是 CUMCM 数学建模全生命周期中的核心技能模块。
**核心定位**：MATLAB 原生图表生成：3D 三维曲面 (surf/mesh)、矢量场流向 (quiver)、Parula 科学色标与系统动力学仿真可视化。

## 2. 为什么需要它 (Why It Matters)

数模竞赛中不可或缺的 MATLAB 原生图表规范，用于处理特定空间几何、偏微分方程解曲面及矢量场。
...
- **生命周期阶段**：Stage S5A (MATLAB 原生高阶图表)
- **前置依赖**：mle-solver
...
| 必需输入工件 | 说明 |
| :--- | :--- |
| `matlab_surface_data.mat` | 阶段执行所依赖的上游工件或输入源 |
| `trajectory_matrix.csv` | 阶段执行所依赖的上游工件或输入源 |

## 5. 输出契约 (Outputs)

| 生成目标工件 | 说明 |
| :--- | :--- |
| `matlab_figure_3d.png` | 本阶段通过严格验证后输出的权威产物 |
| `matlab_figure_3d.svg` | 本阶段通过严格验证后输出的权威产物 |
| `matlab_figure_code.m` | 本阶段通过严格验证后输出的权威产物 |
...
```markdown
使用技能: $matlab-figure
任务目标: 执行 MATLAB 原生图表生成：3D 三维曲面 (surf/mesh)、矢量场流向 (quiver)、Parula 科学色标与系统动力学仿真可视化
输入工件: matlab_surface_data.mat, trajectory_matrix.csv
```
...
```bash
# 查看帮助与参数规范
python skills/06-visualization/matlab-figure/scripts/<script_name>.py --help
```
