...
description: Use when encountering any bug, test failure, or unexpected behavior, before proposing fixes. v4 增强：通用反模式库（魔法数字/方法分歧/盲传结果/阈值遮蔽/过度平滑/假设漂移），适用于所有问题类型。v3 增强：经济建模反模式库。v2 增强：数学建模专属调试模式。
---
