...
def print_report(issues: list[tuple[str, str]]) -> str:
    ...
    if not issues:
        logger.info("  [PASS] 程序自检未发现缺字 / 裁切 / 刻度重叠。")
        logger.info("  >>> 仍需 AI 读图复核感知性问题（见 visual_review.md）。")
        return "PASS"
    ...
    for sev, msg in sorted(issues, key=lambda x: -SEVERITY[x[0]]):
        logger.info(f"  [{sev}] {msg}")
    logger.info(f"  >>> verdict: {verdict}（修完再渲一次 PNG 让 AI 读图复核）")
    return verdict
