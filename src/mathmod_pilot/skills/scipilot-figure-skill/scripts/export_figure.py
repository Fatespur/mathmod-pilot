...
# Attribution
# -----------
# Original-Source : scipilot-figure-skill
# Author          : Haojae
# Repository      : https://github.com/Haojae/scipilot-figure-skill
# License         : MIT  (Copyright (c) 2026 Haojae)
#
...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            logger.info(f"[scipilot-figure-skill] WARNING: skipping {fmt} — "
        ...
        saved.append(path)
        logger.info(f"[scipilot-figure-skill] wrote {path}")
...
def _grayscale_from(fig, basename: str, dpi: int) -> str | None:
    ...
    try:
        ...
        logger.info("[scipilot-figure-skill] Pillow not available; "
    ...
    logger.info(f"[scipilot-figure-skill] wrote {gray_path} (grayscale preview)")
    return gray_path
