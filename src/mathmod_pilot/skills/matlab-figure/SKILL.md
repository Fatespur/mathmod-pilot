---
name: "matlab-figure"
description: "MATLAB数学建模竞赛专属绘图模块。覆盖三维曲面(surf/mesh)、热力图(pcolor)、向量场(quiver)、误差棒(errorbar)、填充区域(patch/fill)、文本标注(text/annot)。支持Simulink系统动力学建模、Vensim集成、专业色标(parula)和论文级图表输出。触发条件：MATLAB绘图、surf、mesh、pcolor、quiver、Simulink、系统动力学可视化、parula色标。"
---
...
- **diagram-generator**：算法流程图/框架图/架构图/Pipeline图（matplotlib.patches）
...
- 题目要求使用 MATLAB
- 需要 Simulink 系统动力学建模
- 需要 MATLAB 独有的 parula 色标（Nature 推荐）
